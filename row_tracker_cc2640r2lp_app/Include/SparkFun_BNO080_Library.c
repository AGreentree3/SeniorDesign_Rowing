...
void IMU_softReset(I2C_Handle handle)
{
    shtpData[0] = 1; //Reset
...
void setFeatureCommand(I2C_Handle handle, uint8_t reportID, uint16_t timeBetweenReports)
{
...
void setFeatureCommand_config(I2C_Handle handle, uint8_t reportID, uint16_t timeBetweenReports, uint32_t specificConfig)
{
    ...
    IMU_sendPacket(handle, CHANNEL_CONTROL, 17);
}
...
bool IMU_receivePacket(I2C_Handle handle)
{
    ...
    i2cTransaction.slaveAddress = BNO080_DEFAULT_ADDRESS;
    ...
    */
    //if (!I2C_transfer(handle, &i2cTransaction)) return false; (THIS BREAKS IT FOR SOME REASON)
...
bool IMU_getData(I2C_Handle handle, uint16_t bytesRemaining)
{
    ...
    while (bytesRemaining > 0)
    {
        ...
        */
        //if (!I2C_transfer(handle, &i2cTransaction)) return false;
        I2C_transfer(handle, &i2cTransaction);
...
bool IMU_sendPacket(I2C_Handle handle, uint8_t channelNumber, uint8_t dataLength)
{
    ...
    if (I2C_transfer(handle, &i2cTransaction)) // returns true if in BLOCKING and successful transfer, or if in CALLBACK
    {
        return true;
    }
